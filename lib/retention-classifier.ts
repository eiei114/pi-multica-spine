...
} from "./idea-session-retention-policy.ts";
...
export interface RetentionDryRunReport { banner: typeof IDEA_SESSION_RETENTION_DRY_RUN_BANNER; eligible: RetentionCandidateReport[]; blocked: RetentionCandidateReport[]; unknown: RetentionCandidateReport[]; exitCode: 0 | 1 | 2 }
...
export async function classifyRetentionCandidate(record: IdeaSessionInventoryRecord, inventoryGeneration: number, options: { now?: Date } = {}): Promise<RetentionCandidateReport> {
  ...
  if (!hasExternalRetentionEvidence(journal, ledgerHash)) return { ...base, state: "blocked", reason: "Durable evidence remains inside the session tree", next: `/skill:idea-status --retention-dry-run --workflow-run-id ${record.workflowRunId} --verbose` };
  const now = options.now ?? new Date();
  if (!isPastFutureReviewWindow(journal, now)) return { ...base, state: "blocked", reason: `Reviewed session has not reached the ${IDEA_SESSION_FUTURE_REVIEW_WINDOW_DAYS}-day future-review window`, next: `/skill:idea-status --retention-dry-run --workflow-run-id ${record.workflowRunId}` };
  if (inventoryGeneration <= 0) return { ...base, state: "unknown", reason: "Stale inventory generation", next: "/skill:idea-status --rebuild" };
  return { ...base, state: "eligible_for_future_review", reason: "Committed parent evidence on Multica with valid ledger past future-review window", next: `/skill:idea-status --retention-dry-run --workflow-run-id ${record.workflowRunId} --verbose` };
}
export async function buildRetentionDryRunReport(records: IdeaSessionInventoryRecord[], inventoryGeneration: number, options: { now?: Date } = {}): Promise<RetentionDryRunReport> {
  const eligible: RetentionCandidateReport[] = []; const blocked: RetentionCandidateReport[] = []; const unknown: RetentionCandidateReport[] = [];
  for (const record of records) { const candidate = await classifyRetentionCandidate(record, inventoryGeneration, { now: options.now }); (candidate.state === "eligible_for_future_review" ? eligible : candidate.state === "blocked" ? blocked : unknown).push(candidate); }
  let exitCode: 0 | 1 | 2 = 0; if (unknown.length) exitCode = 2; else if (blocked.length) exitCode = 1;
  return { banner: IDEA_SESSION_RETENTION_DRY_RUN_BANNER, eligible, blocked, unknown, exitCode };
}
